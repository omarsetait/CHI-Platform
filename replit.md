...
4.  **Audit & FWA Unit Module**: Features AI-powered FWA detection through a 3-phase sequential agent workflow utilizing 36 specialized agents.
    ...
    -   **High-Risk Entity Recompute Job (Task #44)**: Background, eventually-consistent aggregator that runs after every ingestion (`ingest.completed` event) and rolls per-claim FWA detection results into per-entity scores, trends, and evidence in `fwa_high_risk_providers`, `fwa_high_risk_doctors`, `fwa_high_risk_patients`, and `fwa_high_risk_payers` (plus matching `_timeline` rows). Idempotent (UPSERT on entity id; stable `(entityId, batchId)` for timeline rows) with a single-flight in-process guard to collapse concurrent triggers. Manual endpoint: `POST /api/fwa/high-risk-entities/recompute` (optional `jobId`, `since`, `until`, `batchId`). High-risk GET responses (`/api/fwa/high-risk-providers`, `/high-risk-doctors`, `/high-risk-patients`, `/high-risk-payers`) now include a `lastUpdatedAt` timestamp sourced from the seed table. Implemented in `server/services/high-risk-recompute-service.ts`.
    -   **Smart Batch Claim Ingestion Pipeline (Task #41)**: Async job-based ingestion of Excel/CSV/JSON claim files. Auto-maps source columns to canonical claim fields (heuristic + optional LLM-assisted mapping), normalizes and validates rows (partial rows are persisted with per-row missing-field markers and ineligible-engine reasons), persists to `claims_v2` with auto-stub of missing FK parents, runs all 5 detection engines per claim with per-engine eligibility gating, and emits an internal `ingest.completed` event for downstream consumers (test-case generator, detection engine page, high-risk recompute). Backed by `fwa_ingest_jobs` and `fwa_ingest_rows` tables. Endpoints: `POST /api/fwa/ingest` (multipart `file` or JSON `inlineRows`; optional `requireConfirmation` pauses the worker after auto-mapping at status `awaiting_confirmation`), `POST /api/fwa/ingest/:jobId/confirm` (resume a paused job, optionally with additional `mappingOverrides`), `GET /api/fwa/ingest/:jobId` (status + summary), `GET /api/fwa/ingest/:jobId/results` (paginated per-row results with per-engine scores AND `detectionEvidence` containing rule/statistical/unsupervised/rag_llm/semantic findings joined from `fwa_detection_results`). Implemented in `server/services/fwa-ingest-pipeline.ts` and `server/routes/fwa-ingest-routes.ts`.

        **Operational constraints (bounded memory).** CSV sources are streamed line-by-line and are effectively unbounded in size. Excel and JSON sources are loaded whole-document by their parsers; to keep memory bounded the pipeline rejects oversize files with a clear, surfaceable error (job `status="failed"` with an explanatory `errorMessage`). Defaults: 50 MB Excel (`FWA_INGEST_MAX_EXCEL_BYTES`), 100 MB JSON (`FWA_INGEST_MAX_JSON_BYTES`). For larger files, convert to CSV (preferred) or raise the env-var ceiling if the deployment can afford the working set. Upload staging directory: `FWA_INGEST_STAGE_DIR` (default `$TMPDIR/fwa-ingest-stage`). All non-terminal jobs are recovered on server boot via `recoverInFlightJobs()`; jobs paused at `awaiting_confirmation` are intentionally not auto-resumed (they wait for the explicit `/confirm` call), and the persisted `requireConfirmation` flag is honored so recovered jobs re-arm the pause behavior.
