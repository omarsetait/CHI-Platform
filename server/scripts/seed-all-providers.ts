...
// Providers that need claims - using exact IDs from Dream Reports
const dreamReportProviders = [
    { id: "97817751-fb40-4c57-86e3-8e234a61132d", name: "Dallah Health Clinic" },
    { id: "c33c5199-671f-4a98-84c8-19f36727963a", name: "International Medical Center" },
...
const servicesCatalog = {
    ...
    outpatient: [
        { code: "99213", description: "Office visit - low complexity", minPrice: 150, maxPrice: 350 },
        { code: "99214", description: "Office visit - moderate complexity", minPrice: 250, maxPrice: 500 },
        { code: "99215", description: "Office visit - high complexity", minPrice: 400, maxPrice: 700 },
    ...
    emergency: [
        { code: "99284", description: "Emergency dept visit - moderate", minPrice: 600, maxPrice: 1500 },
        { code: "99285", description: "Emergency dept visit - high severity", minPrice: 1200, maxPrice: 3000 },
        { code: "99291", description: "Critical care - first hour", minPrice: 2000, maxPrice: 5000 },
        { code: "12001", description: "Simple wound repair", minPrice: 300, maxPrice: 800 },
        ...
        { code: "96374", description: "IV push injection", minPrice: 100, maxPrice: 250 },
    ],
    pharmacy: [
...
async function seedAllProviders() {
    ...
    for (const provider of dreamReportProviders) {
        console.log(`\nSeeding claims for: ${provider.name}`);

        // Generate 25-40 claims per provider
        const numClaims = Math.floor(Math.random() * 16) + 25;
        ...
        for (let i = 0; i < numClaims; i++) {
            ...
            const claim = {
                id: claimId,
                claimNumber: `CLM-${now.getFullYear()}-${String(totalClaims + i + 200).padStart(5, "0")}`,
            ...
            try {
                ...
                console.error(`Error inserting claim: ${err.message}`);
            }

            // Insert services
            for (const svc of claimServices) {
                try {
                    await client.query(`
                        INSERT INTO ihop.fwa_claim_services (
                            ...
                            adjudication_status, approval_status, violations, denial_reason
                    ...
                    `, [
                        ...
                        svc.adjudicationStatus, svc.approvalStatus, svc.violations, svc.denialReason
                    ]);
                    totalServices++;
                } catch (err: any) {
        ...
        console.log(`  - Created claims for ${provider.name}`);
    }

    console.log(`\n========================================`);
    console.log(`Total Claims Created: ${totalClaims}`);
...
seedAllProviders().catch(console.error);
