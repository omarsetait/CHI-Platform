...
export function mapRowToClaim(row: ClaimImportRow, index: number): any {
  ...
  return {
    ...
    serviceDate: parseDate(row.StartDate) || parseDate(row.OccurrenceDate) || now,
    ...
    patientShare: parseDecimal(row.PatientShareLC),
    status: "pending",
    primaryDiagnosis: row.PrimaryDiagnosis || "",
    icdCodes: parseArray(row.SecondaryICDs) || (row.PrimaryDiagnosis ? [row.PrimaryDiagnosis] : null),
    cptCodes: row.ServiceCode ? [row.ServiceCode] : null,
    description: row.ServiceDescription || row.ProviderServiceDescription || "",
    specialty: row.Speciality || row.SpecialityCode || null,
    ...
    city: row.ProviderCity || null,
    coverageRelationship: row.CoverageRelationship || null,
    groupNo: row.GroupNo || null,
    isNewborn: parseBoolean(row.NewbornFlag),
    isChronic: parseBoolean(row.ChronicFlag),
    isPreExisting: parseBoolean(row.PreExistingFlag),
    isPreAuthorized: parseBoolean(row.IsPreAuthorized),
    isMaternity: parseBoolean(row.MaternityFlag),
    preAuthRef: row.PreAuthorizationID || null,
    preAuthStatus: row.PreAuthorizationStatus || null,
    preAuthIcd10s: parseArray(row.PreAuthorizationIcd10s),
    ...
    dischargeDiagnosis: parseArray(row.DischargeDiagnosisCodes),
    onAdmissionDiagnosis: row.OnAdmission ? [row.OnAdmission] : null,
    source: row.Source || null,
    aiStatus: row.AIStatus || null,
    validationResults: row.Validation ? { raw: String(row.Validation), comment: row.Comment || null } : null,
    flagged: false,
    flagReason: null,
    outlierScore: "0.0000",
  };
...
export async function getImportStats(): Promise<{
  ...
  const [totalResult, todayResult, typesResult] = await Promise.all([
    db.execute(sql`SELECT COUNT(*)::int as count FROM claims_v2`),
    db.execute(sql`SELECT COUNT(*)::int as count FROM claims_v2 WHERE registration_date >= ${today}`),
    db.execute(sql`
